...
import { LoadoutStatsCalculator } from "../../../domain/services/LoadoutStatsCalculator";
import { LoadoutVisualHelpers } from "../../../domain/services/LoadoutVisualHelpers";
...
export default function CurrentLoadout({ membershipType, userId, name, seasonHash, rank, light, charStats }) {
            ...
            try {
                ...
                const itemDetails = await Promise.all(response.equipment.data.items.map(async (item) => {
                    ...
                    if ([3, 4, 5, 6, 7].includes(response.equipment.data.items.indexOf(item))) {
                        ...
                        armorStats = LoadoutStatsCalculator.getArmorStats(itemResponse, investmentStats, armorStats, itemD.instance?.data?.gearTier);
                        const { modifierPerks, designPerks } = LoadoutStatsCalculator.sortByArtificePerk(perks);
                        perks = {
                            modifierPerks: modifierPerks,
                            cosmeticPerks: designPerks
                        };
                        bgColor = LoadoutVisualHelpers.getRarityColor(itemResponse.inventory.tierType);
                        bgMasterwork = [8, 5, 4, 9].some(value => item.state && item.state === value) ? masterworkHeader : null;
                        armorCategory = LoadoutVisualHelpers.getArmorCategory(itemResponse.itemTypeDisplayName, itemResponse.classType);
                        ...
                        weaponStats = await getWeaponStats(itemResponse, investmentStats, LoadoutStatsCalculator.getWeaponLevel(itemD.plugObjectives.data.objectivesPerPlug), itemD.instance?.data?.gearTier);
                        weaponStats = LoadoutStatsCalculator.colorStats(itemResponse, perks.filter(perk => perk != null), weaponStats, itemD.instance?.data?.gearTier)
                        const { modifierPerks, cosmeticPerks } = LoadoutStatsCalculator.sortWeaponPerks(perks);
                        perks = {
                            modifierPerks: modifierPerks,
                            cosmeticPerks: cosmeticPerks
                        };
                        tracker = LoadoutStatsCalculator.getTrackerKills(perks.cosmeticPerks?.tracker[0]?.plugHash, itemD.plugObjectives?.data?.objectivesPerPlug)
                        ...
                        secondaryBgImg = LoadoutVisualHelpers.getSecondaryBgImg(itemResponse.talentGrid?.hudDamageType);
                        ...
                        bgColor = LoadoutVisualHelpers.getRarityColor(itemResponse.inventory.tierType);
                        bgMasterwork = [8, 5, 4, 9].some(value => item.state && item.state === value) ? masterworkHeader : null;
                        ghostEnergy = LoadoutVisualHelpers.getGhostEnergy(item.state, perks);
                    }
                    return {
                        ...
                        tierColor: LoadoutVisualHelpers.getTierColor(itemD.instance?.data?.gearTier),
    ...
    async function getWeaponStats(item, perksinvestmentStats, weaponLevel, tier) {
        ...
        weaponStats = LoadoutStatsCalculator.interpoledStats(weaponStats, interpolatingStats);//Se "normalizan" las stats mediante interpolacion
        return weaponStats;
    }
    ...
    async function getSeal(char, manifest) {
        ...
        const matchingNode = Object.values(metricsData).find(node => node.completionRecordHash === char.titleRecordHash);
        setSeal({
